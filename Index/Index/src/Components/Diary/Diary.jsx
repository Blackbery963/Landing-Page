...
import diaryBackground from '/home/swarnadip/Documents/Index/Index/Index/src/Components/Diary/Diary-images/1d19d578-7d3a-454b-9ce3-2dae3fd63c7b.jpg';
...
function Diary() {
  ...
  // const [hasCustomMonthBg, setHasCustomMonthBg] = useState(false);
            ...
            {[
              ...
              const hasBackground = monthsBackgrounds[index] !== null;
               return (
                <div
                key={index}
                className="bg-white shadow-lg rounded-lg p-4 flex flex-col items-center justify-center cursor-pointer hover:scale-105 transition-transform border border-gray-300"
                style={{
                  ...
                  color: monthsBackgrounds[index] ? 'white' : 'black',
                }}
              >
                <h3 className={`text-base sm:text-xl font-semibold font-Playfair text-center ${hasBackground ? 'text-white' : 'text-black'}`}>{item.month}</h3>
                <p className={`text-base sm:text-lg font-normal font-Newsreader text-center ${hasBackground ? 'text-white' : 'text-slate-800'}`}>
                  {item.description}
                </p>
                {/* File input in top-right corner */}
                {/* <label
                  htmlFor={`month-upload-${index}`}
                  className="absolute top-2 right-2 bg-blue-600 text-white px-3 py-1 rounded-md cursor-pointer"
                >
                  Upload
                  <input
                    type="file"
                    id={`month-upload-${index}`}
                    accept="image/*"
                    className="hidden"
              ...
              */}
               <input type="file" accept="image/*" className="hidden" id={`upload-${index}`} onChange={(e) => handleMonthBackgroundImage(e, index)} />
                <label htmlFor={`upload-${index}`} className="absolute top-2 right-2 bg-transparent border border-gray-900 text-black p-1 rounded-md cursor-pointer">
                  <FaArrowUp />
                </label>
                <Link to={item.to}>
                <button  className={`hidden md:block border  px-2 font-Playfair rounded-md mt-4 backdrop-blur-md ${hasBackground ? 'text-white' : 'text-slate-800'} ${hasBackground ? 'border-slate-100' : 'border-gray-600'}`}>
                  Explore
                </button>
                </Link>
