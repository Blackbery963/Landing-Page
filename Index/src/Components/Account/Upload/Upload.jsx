...
function UploadComponent({ index, onImageChange, onRemoveImage, image, classification, error, onSubmit }) {
  const [selectedTag, setSelectedTag] = useState("");
  const [customTag, setCustomTag] = useState("#");
  const [isCustom, setIsCustom] = useState(false);
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      if (file.size > MAX_FILE_SIZE) {
        onImageChange(index, null, file, "", "File size exceeds 50MB.");
        return;
      }
      const reader = new FileReader();
      reader.onloadend = () => {
      ...
      >
        ...
        <div className="w-full flex flex-col gap-1 items-start">
          ...
          <select
            ...
            className="h-10 lg:w-[60%] md:w-[75%] sm:w-[85%] w-[95%] px-3 border border-gray-500 rounded-md bg-transparent font-Funnel outline-none text-white focus:border-none focus:ring-1 focus:ring-[#fc8e97] cursor-pointer"
          >
            <option value="" disabled selected hidden>Choose medium...</option>
            <option className="bg-[#990931] text-white">Oil</option>
            <option className="bg-[#990931] text-white">Watercolor</option>
            <option className="bg-[#990931] text-white">Digital</option>
            <option className="bg-[#990931] text-white">Acrylic</option>
...
function Upload() {
  ...
  const handleImageChange = async (index, image, imageFile, classification, error) => {
    ...
    if (imageFile) {
      ...
      try {
        ...
        setUploads((prev) => {
          const newUploads = [...prev];
          newUploads[index].classification = data.result || "Unknown";
          return newUploads;
        });
      } catch (error) {
        setUploads((prev) => {
  ...
  const handleSubmit = async (e, index) => {
    ...
    const description = formData.get("description");
    const medium = formData.get("medium");
    const tags = formData.get("tags") || uploads[index].customTag || "#Default"; // Fallback if tags not set

    const imageFile = uploads[index].imageFile;
    ...
    try {
      ...
      const document = await databases.createDocument(
        ...
        ID.unique(),
        {
          ...
          classification: uploads[index].classification,
        }
      );
      alert("Upload successful!");
      console.log("Stored in Appwrite:", document);
  ...
  return (
    <div className="min-w-screen min-h-screen bg-[#ffeeee]">
      <header className="h-[100px] w-full bg-gradient-to-l from-[#3c25267c] via-[#5e3b4d75] to-[#d9a1bf80] backdrop-blur-md flex items-center justify-between px-4 md:px-6 shadow-lg text-white fixed top-0 z-50">
        ...
        <div className="flex items-center justify-center gap-x-2">
          <Link to={"/"}><button className="lg:px-4 px-2 py-1 bg-[#a6565d] hover:bg-[#c68a9a] hover:text-[#2d1f23] rounded-md font-Playfair text-white border border-gray-400 text-[18px]"><FaHome className="text-xl sm:hidden" /><span className="hidden sm:inline">Home</span></button></Link>
          <Link to={"/About"}><button className="lg:px-4 px-2 py-1 bg-[#a6565d] hover:bg-[#c68a9a] hover:text-[#2d1f23] rounded-md font-Playfair text-white border border-gray-400 text-[18px]"><FaInfoCircle className="text-xl sm:hidden" /><span className="hidden sm:inline">About</span></button></Link>
          <Link to={"/Account"}><button className="lg:px-4 px-2 py-1 bg-[#a6565d] hover:bg-[#c68a9a] hover:text-[#2d1f23] rounded-md font-Playfair text-white border border-gray-400 text-[18px]"><FaUser className="text-xl sm:hidden" /><span className="hidden sm:inline">Account</span></button></Link>
          <Link to={"/Diary"}><button className="lg:px-4 px-2 py-1 bg-[#a6565d] hover:bg-[#c68a9a] hover:text-[#2d1f23] rounded-md font-Playfair text-white border border-gray-400 text-[18px]"><FaBook className="text-xl sm:hidden" /><span className="hidden sm:inline">Diary</span></button></Link>
        </div>
      </header>
      <div className="w-full flex flex-col items-center mt-[100px] pt-12">
        <div onClick={addNewUpload} className="h-[50px] w-[50px] bg-black/10 hover:bg-black/25 border border-gray-400 rounded-lg flex items-center justify-center cursor-pointer">
          <FaPlus />
        </div>
      </div>
      {uploads.map((upload, index) => (
